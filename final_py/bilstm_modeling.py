...
def plotHistory(history) :
    """
    epoch에 따른 train data, test data의 정확도 추이 그래프 작성
    ...
    parameters
        history : 데이터를 학습시킨 모델
    ...
    ...
    # 그래프 그리기
    metrics[['Training_Accuracy', 'Test_Accuracy']].plot()
...
def showReport(model, X_test_pad, y_test) :
    """
    결과 리포트 발행
    ...
    parameters
        model : 모델
        X_test_pad : 독립변수 시험데이터 (dataFrame; shape[0.3*n, 1])
        y_test : 종속변수 시험데이터 (dataFrame; shape[0.3*n, 1])
    ...
    returns
        None
...
def saveModel(table, model) : 
    ...
    """
    # 모델 저장
    with open('BiLSTM_'+table++'.pkl', 'wb') as f:
        pickle.dump(model, f)
